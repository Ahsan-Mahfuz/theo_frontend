...
export default function RefusedTaskPage() {
  const router = useRouter();
  const t = useTranslations('Tasks');
  const c = useTranslations('Common');
  const property = MOCK_ACCOMMODATIONS[0];
  const cleaners = [MOCK_HOUSEKEEPERS[0], MOCK_HOUSEKEEPERS[1], MOCK_HOUSEKEEPERS[0]];
  ...
  return (
    <div className="w-full flex items-center justify-center min-h-[70vh] animate-in fade-in slide-in-from-right-8 duration-500 py-10">
      <div className="w-full max-w-[600px] bg-white rounded-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-gray-100 p-10 flex flex-col items-center">
        ...
        {/* Badge */}
        <div className="w-14 h-14 bg-[#FEE2E2] rounded-full flex items-center justify-center mb-6">
        ...
        {/* Title */}
        <h2 className="text-[16px] font-bold text-gray-900 mb-2">{t('refusedTitle')}</h2>
        ...
        <div className="w-full max-w-[400px] flex flex-col gap-6">
          ...
          {/* Nearest Housekeeper Block */}
          <div className="flex flex-col gap-3 mt-4">
             <span className="text-[13px] font-bold text-gray-900">{t('nearestHousekeeper')}</span>
                   ...
                   <div className="flex items-center gap-3">
                     ...
                     <div className="flex flex-col">
                       ...
                       <div className="flex items-center gap-1 text-gray-500">
                         ...
                         <span className="text-[10px]">Paris 11e</span>
                       </div>
                     </div>
                   </div>
                   <button className="px-5 h-8 bg-gray-100 text-gray-700 text-[11px] font-medium rounded-lg hover:bg-gray-200 transition-colors">
                     {c('send')}
                   </button>
                 </div>
               ))}
