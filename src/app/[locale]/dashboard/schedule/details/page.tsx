...
export default function ScheduleDetailsPage() {
  ...
  // If coming from Recommended Cleaning, prefill the times if empty.
  useEffect(() => {
    if (isRecommended && !data.date) {
      updateData({
        date: '2026-05-22',
        checkInTime: '12:30',
        checkOutTime: '10:00',
  ...
  const handleContinue = () => {
    if (isFormComplete) {
      router.push('/dashboard/schedule/payment');
    }
  };
    ...
    <div className="w-full flex flex-col animate-in fade-in slide-in-from-right-8 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12">
        ...
        <div className="flex flex-col">
           ...
           {!isFormComplete ? (
             <div className="w-full max-w-[200px] h-10 bg-gray-100 rounded-lg flex items-center justify-center text-[12px] text-gray-500 font-medium mx-auto lg:mx-0 mt-4 cursor-not-allowed">
               {t('assignCleaner')}
             </div>
           ) : (
        ...
        <div className="flex flex-col">
           ...
           <div className="mt-auto">
             <button
               ...
               className={`w-full lg:w-auto lg:ml-auto px-16 h-11 rounded-lg text-[13px] font-medium transition-colors ${isFormComplete ? 'bg-black text-white hover:bg-gray-800' : 'bg-gray-400 text-white cursor-not-allowed'}`}
             >
               {c('next')}
             </button>
           </div>
